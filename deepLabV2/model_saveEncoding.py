...
class Model_msc(object):
    ...
    def test(self):
        ...
        checkpointfile = tf.train.latest_checkpoint("./model_multigpu/")
        ...
        for step in range(self.conf.valid_num_steps):
            ...
            # To save encoder outputs
            outputs, name = self.sess.run([self.outputs, self.name])
            np.save("encoder_output/"+name[-15:-4].decode("utf-8"), outputs[0])
            ...
            with tf.device(gpu_list[i]):
                ...
                with tf.name_scope("create_inputs"):
                    reader = ImageReader(
                        ...
                        self.coord,
                        get_encoding=False)
                    self.image_batch, self.label_batch, self.names = reader.dequeue(self.conf.batch_size)
                    print("encoding batch")
                    self.encoding_batch = []
                    for name in self.names:
                        self.encoding_batch.append(np.load("./encoder_output/"+name+".npy"))
                    print(self.encoding_batch)
                ...
                zip_decoder_b.append(list(zip(grads_decoder_w, decoder_w_trainable)))
                zip_decoder_w.append(list(zip(grads_decoder_b, decoder_b_trainable)))
    ...
    def test_setup(self):
        ...
        with tf.name_scope("create_inputs"):
            reader = ImageReader(
                ...
                IMG_MEAN,
                self.coord)
            image, label, name = reader.image, reader.label, reader.name # [h, w, 3 or 1]
        ...
        print("raw_output_05")
        print(raw_output05)
        self.outputs = [raw_output100, raw_output075, raw_output05]
        self.name = name
        '''
        raw_output = tf.reduce_max(tf.stack([raw_output100,
                                    tf.image.resize_images(raw_output075, tf.shape(raw_output100)[1:3,]),
