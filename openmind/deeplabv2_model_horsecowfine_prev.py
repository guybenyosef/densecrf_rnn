        ...
        for i in range(len(gpu_list)):
            with tf.device(gpu_list[i]):
                ...
                accum_grads = [tf.Variable(tf.zeros_like(v.initialized_value()),
                                           trainable=False) for v in  decoder_w_trainable + decoder_b_trainable + crf_trainable] #encoder_trainable +
                ...
                # Compute grads
                grads = tf.gradients(self.reduced_loss, decoder_w_trainable + decoder_b_trainable + crf_trainable) #encoder_trainable + 
