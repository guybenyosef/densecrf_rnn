...
class Deeplab_v2(object):
    ...
    Deeplab v2 pre-trained model (pre-trained on MSCOCO + PASCAL_train+val) ('deeplab_resnet.ckpt')
    """
    def __init__(self, inputs, num_classes, phase, rescale075=False, rescale05=False, crf_type='crf'):
        self.inputs = inputs
    ...
    def build_network(self):
        ...
        if self.crf_type == 'crf':
            self.outputs =CrfRnnLayer(image_dims=crf_dims,#crf_dims, (self.image_height, self.image_width)
                                      num_classes=self.num_classes,
                                      theta_alpha=160.,
                                      theta_beta=90.,
                                      theta_gamma=3.,
                                      num_iterations=5,
                                      name='crfrnn')([self.resized_decoding, self.raw_inputs])#([self.resized_decoding, self.raw_images])
        else:
