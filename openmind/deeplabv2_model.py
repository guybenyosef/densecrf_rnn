...
class Model_msc(object):
    ...
    def train(self):
        ...
        for i in range(len(gpu_list)):
            with tf.device(gpu_list[i]):
                if self.conf.pretrain_file is not None:
                    ...
                    #checkpointfile = tf.train.latest_checkpoint("./../Deeplab-v2--ResNet-101--Tensorflow/model_msc_bs10/")
                    checkpointfile = tf.train.latest_checkpoint("./model_crf/")
        ...
        for step in range(self.conf.num_steps+1):
            ...
            feed_dict = { self.curr_step : step + 20000} # finetuning from 20K steps
            loss_value = 0
    ...
    def test(self):
        ...
        checkpointfile = tf.train.latest_checkpoint("./model_crf/")
        #checkpointfile = './model_crf/model.ckpt-10000'
