...
from network_horsecowfine import *
...
class Model_msc(object):
    ...
    def test(self):
        ...
        #checkpointfile = tf.train.latest_checkpoint("./model_horsecowfine_crfSP/")
        checkpointfile = tf.train.latest_checkpoint(self.conf.modeldir)
        #checkpointfile = './model_horsecowfine_crf_noEncoder/model.ckpt-200'
    ...
    def predict(self):
        ...
        image_list, _, sp = read_labeled_image_list('', self.conf.test_data_list)
        #image_list, _, sp = read_labeled_image_list(‘’, self.conf.test_data_list)
        ...
        for step in range(self.conf.test_num_steps):
            ...
            # For debugging the intermediate outputs:
    ...
    def train_setup(self):
            ...
            with tf.device(gpu_list[i]):
                ...
                with tf.name_scope("create_inputs"):
                    reader = ImageReader(
                        ...
                        IMG_MEAN,
                        self.coord)
                    self.image_batch, self.label_batch, self.sp_batch = reader.dequeue(self.conf.batch_size)
                    ...
                    sp_batch_075 = tf.image.resize_images(self.sp_batch, [int(h * 0.75), int(w * 0.75)])
                    sp_batch_05 = tf.image.resize_images(self.sp_batch, [int(h * 0.5), int(w * 0.5)])
                ...
                with tf.variable_scope('', reuse=False):
                    if self.conf.crf_type == 'crf':
                        ...
                        net = Deeplab_v2(self.image_batch, self.conf.num_classes, True, rescale075=False, rescale05=False, crf_type=self.conf.crf_type, superpixels=self.sp_batch)
                ...
                restore_var = [v for v in tf.global_variables() if ('fc' not in v.name and 'crfrnn' not in v.name)] # when don't want to train using previous crf weights
                ...
                # Check
                assert(len(all_trainable) == len(encoder_trainable) + len(decoder_trainable) + len(crf_trainable))  #+ len(encoder_trainable)
                ...
                loss = tf.nn.softmax_cross_entropy_with_logits(logits=raw_prediction, labels=tf.reshape(label_proc[0], (h*w, self.conf.num_classes)))
                ...
                loss100 = tf.nn.softmax_cross_entropy_with_logits(logits=raw_prediction100, labels=tf.reshape(label_proc[0], (h*w, self.conf.num_classes)))
                # NOTE used to be loss=tf.nn.softmax_cross_entropy_with_logits_v2
                ...
                accum_grads = [tf.Variable(tf.zeros_like(v.initialized_value()),
                                           trainable=False) for v in  encoder_trainable + decoder_w_trainable + decoder_b_trainable + crf_trainable] #encoder_trainable +
                ...
                # Compute grads
                grads = tf.gradients(self.reduced_loss,encoder_trainable+ decoder_w_trainable + decoder_b_trainable + crf_trainable) #encoder_trainable + 
        ...
        with tf.control_dependencies(update_ops):
            self.train_op = tf.group(train_op_conv, train_op_fc_w, train_op_fc_b, train_op_crf) # train_op_conv
    ...
    def test_setup(self):
        ...
        self.image_batch, self.label_batch, self.sp_batch = tf.expand_dims(image, dim=0), tf.expand_dims(label, dim=0), tf.expand_dims(sp, dim=0)
        h_orig, w_orig = tf.to_float(tf.shape(self.image_batch)[1]), tf.to_float(tf.shape(self.image_batch)[2])
        h_sp, w_sp = tf.to_float(tf.shape(self.sp_batch)[1]), tf.to_float(tf.shape(self.sp_batch)[2])
        for i in range(1):
        ...
        with tf.variable_scope('', reuse=False):
            if self.conf.crf_type == 'crf':
                ...
                net = Deeplab_v2(self.image_batch, self.conf.num_classes, False, rescale075=False, rescale05=False,crf_type=self.conf.crf_type, superpixels=self.sp_batch)
        '''
        with tf.variable_scope('', reuse=True):
